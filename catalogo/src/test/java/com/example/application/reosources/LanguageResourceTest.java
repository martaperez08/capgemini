...
class LanguageResourceTest {
	...
	void testGetAllString() throws Exception {
		...
		List<LanguageShort> lista = new ArrayList<>(
		        Arrays.asList(new LanguageShortMock(1, "Chino"),
		        		new LanguageShortMock(2, "Japones"),
		        		new LanguageShortMock(3, "Thailandes")));
	...
	void testGetAllPageable() throws Exception {
		...
		when(srv.getByProjection(PageRequest.of(0, 20), LanguageShort.class))
			.thenReturn(new PageImpl<>(lista));
		mockMvc.perform(get("/api/language/v1").queryParam("page", "0"))
			.andExpectAll(
				status().isOk(), 
				content().contentType("application/json"),
	...
	void testGetOne() throws Exception {
		...
		mockMvc.perform(get("/api/language/v1/{id}", id))
