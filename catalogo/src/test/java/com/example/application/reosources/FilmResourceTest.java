...
class FilmResourceTest {
		...
		void testGetAllString() throws Exception {
			List<FilmShort> lista = new ArrayList<>(
			        Arrays.asList(new FilmShortMock(1, "Pepito Grillo"),
			        		new  FilmShortMock(2, "Carmelo Coton"),
			        		new  FilmShortMock(3, "Capitan Tan")));
			when(srv.getByProjection(FilmShort.class)).thenReturn(lista);
			mockMvc.perform(get("/api/film/v1").accept(MediaType.APPLICATION_JSON))
				.andExpectAll(
						status().isOk(), 
		...
		void testGetAllPageable() throws Exception {
			...
			mockMvc.perform(get("/api/film/v1").queryParam("page", "0"))
				.andExpectAll(
					status().isOk(), 
					content().contentType("application/json"),
		...
		void testGetOne() throws Exception {
			...
			mockMvc.perform(get("/api/film/v1/{id}", id))
				.andExpect(status().isOk())
		        .andExpect(jsonPath("$.id").value(id))
		        .andExpect(jsonPath("$.descr").value(ele.getDescription()))
		        .andDo(print());
		}
		@Test
		...
		void testCreate() throws Exception {
			...
			mockMvc.perform(post("/api/film/v1/")
				.contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(FilmDTO.from(ele)))
				)
				.andExpect(status().isCreated())
		        .andExpect(header().string("Location", "http://localhost/api/film/v1/1"))
		        .andDo(print())
		        ;
		}
